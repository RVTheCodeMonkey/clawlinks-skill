...
for topic in sorted(by_topic.keys()):
  ...
  <button class="delete-btn" onclick="copyRemove('{h_url}')">&#128465;</button>
</div>\n'''
    groups_html += '</div>\n'
...
function copyRemove(url) {{
  navigator.clipboard.writeText('remove ' + url).then(() => {{
    const t = document.getElementById('toast');
    t.textContent = 'Copied! Open ClawLinks to paste and send';
